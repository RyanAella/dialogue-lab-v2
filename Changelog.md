...
### Changed
- **Variant System**: Implemented centralized variant management for different deployment configurations (practice, simulation, default). Variants are now automatically detected via URL path (production) or hash parameter (local development), enabling a single codebase to serve multiple configurations.
- **Mode Selection**: Mode is now automatically determined by the active variant. Single-mode variants (practice = transformation, simulation = simulation/roleplay) automatically select their mode, eliminating the need for manual mode selection.
- **Dropdown Filtering**: Scenario and exercise dropdowns are now filtered based on the current variant's allowed modes. Practice edition shows only transformation exercises, simulation lab shows only simulation exercises.
- **Path Resolution**: Added `<base href="/dialogue_lab/">` to resolve all asset paths correctly regardless of deployment path, fixing 404 errors for JavaScript modules and static files.
- **Mode Dropdown**: Removed the mode selection dropdown as it is now automatically determined by the variant.
